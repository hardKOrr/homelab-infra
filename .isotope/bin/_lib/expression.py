"""Native Expression protocol mapping, briefing, and durable evidence recording."""
...
def protocol() -> dict[str, Any]:
    ...
    if relative != "expression/protocol.json":
        raise IsotopeError("protocol-invalid", "The Expression protocol is absent from the reaction catalog.", EXIT_MALFORMED)
    ...
    if value.get("id") != REACTION or value.get("version") != "1" or value.get("transport") != "native-only":
        raise IsotopeError("protocol-invalid", "The Expression protocol root is invalid.", EXIT_MALFORMED)
    return value
...
def _select_option(project, host: str, model: str | None) -> str:
    ...
    if len(choices) != 1:
        raise IsotopeError(
            ...
            "The requested host/model does not resolve to one enabled Expression option.",
            EXIT_REFUSED,
...
def _coordinates(project, slug: str | None) -> dict[str, Any]:
    if slug is None:
        ...
        if operating is None:
            raise IsotopeError("no-armed-operation", "An unarmed Expression requires an explicit slug.", EXIT_REFUSED)
        slug = operating["slug"]
    return {"slug": slug}
...
def _snapshot_state(manifest_value: dict[str, Any]) -> dict[str, dict[str, Any]]:
    state: dict[str, dict[str, Any]] = {}
    for kind in ("index", "tracked", "untracked"):
        for entry in manifest_value.get(kind, []):
            state.setdefault(entry["path"], {})[kind] = {key: value for key, value in entry.items() if key != "path"}
    return state


def _assert_bounds(accepted: dict[str, Any], current: dict[str, Any], target_paths: set[str]) -> None:
    if current["head_commit"] != accepted["head_commit"] or current["base_commit"] != accepted["base_commit"]:
        raise IsotopeError("worktree-moved", "The Git history moved after acceptance; Expression judges the accepted worktree.", EXIT_CONFLICT, {"accepted_head": accepted["head_commit"], "current_head": current["head_commit"]})
    before = _snapshot_state(accepted)
    after = _snapshot_state(current)
    drift = sorted(path for path in set(before) | set(after) if before.get(path) != after.get(path))
    out_of_bounds = [path for path in drift if path not in target_paths]
    if out_of_bounds:
        raise IsotopeError(
            "expression-out-of-bounds",
            "The worktree changed outside the declared Atlas targets since acceptance.",
            EXIT_REFUSED,
            {"paths": out_of_bounds, "targets": sorted(target_paths)},
        )
...
def _resolve(project, coordinates: dict[str, Any]) -> dict[str, Any]:
    ...
    if located.stage != "flux":
        raise IsotopeError("wrong-stage", "Expression requires a flux specimen.", EXIT_REFUSED, {"stage": located.stage})
    ...
    if operating is None or operating["slug"] != coordinates["slug"] or operating["specimen_revision"] != specimen_revision:
        raise IsotopeError("no-armed-operation", "Expression requires the exact armed specimen revision.", EXIT_REFUSED, {"slug": coordinates["slug"]})
    if gitops.current_branch(project) != operating["branch"]:
        raise IsotopeError("operating-drift", "Expression requires the armed branch to be current.", EXIT_CONFLICT)
    outcome = value.get("outcome")
    if outcome is None:
        raise IsotopeError("outcome-missing", "Expression requires the specimen's outcome record.", EXIT_REFUSED, {"slug": coordinates["slug"], "next_action": "record the outcome with specimen outcome set"})
    isotope_docs.require_mapped_targets(project, outcome["doc_targets"])
    last_acceptance = value["acceptances"][-1]
    accepted_snapshot = last_acceptance.get("acceptance_snapshot")
    if accepted_snapshot is None:
        raise IsotopeError("acceptance-snapshot-missing", "Expression bounds require the latest acceptance's canonical snapshot.", EXIT_REFUSED, {"acceptance": last_acceptance["id"]})
    target_paths = {target["path"] for target in outcome["doc_targets"]}
    current_snapshot, _ = gitops.review_snapshot(project, operating["base_commit"])
    _assert_bounds(accepted_snapshot, current_snapshot, target_paths)
    targets = []
    for target in outcome["doc_targets"]:
        section, _ = isotope_docs.section(project, target["path"], target["section_id"])
        targets.append({"concept": target["concept"], "path": target["path"], "section_id": target["section_id"], **{key: value_ for key, value_ in section.items() if key != "path"}})
    doc_revisions: dict[str, str] = {}
    for entry in isotope_docs.map_entries(project)[0]:
        if entry["path"] in target_paths or f"doc:{entry['path']}" in doc_revisions:
            continue
        _, source = isotope_docs.section(project, entry["path"], entry["section_id"])
        doc_revisions[f"doc:{source['path']}"] = source["revision"]
    return {
        "specimen": value,
        ...
        "outcome": outcome,
        "targets": targets,
        "acceptance_snapshot": accepted_snapshot,
        "already_complete": outcome.get("expression") is not None,
...
def _slot_metadata(resolved: dict[str, Any]) -> list[dict[str, Any]]:
    counts = {
        ...
        "operating": 1,
        "answers": 0,
        "result_destination": 1 if resolved["already_complete"] else 0,
    }
    ...
    try:
        ...
        if after is not None:
            ...
            if predecessor["status"] == "blocked" and predecessor["blocking_condition"] is not None:
                ...
                if facts.get("expression_state") == revision(resolved["source_revisions"]):
        ...
        if resolved["already_complete"]:
            return {"state": "complete", "coordinates": coordinates, "outcome": "expressed", "entity": {"kind": "outcome", "id": "outcome", "revision": revision(resolved["outcome"])}, "slots": _slot_metadata(resolved)}
...
def open_invocation(project, slug: str | None, *, host: str, model: str | None, after: str | None) -> dict[str, Any]:
    ...
    if result["state"] != "ready":
        raise IsotopeError("agent-not-ready", "Expression cannot open until inspection is ready.", EXIT_REFUSED, result)
...
def _assert_sources(project, invocation: dict[str, Any]) -> dict[str, Any]:
    ...
    if mismatches:
        raise IsotopeError("stale-expression-source", "A frozen Expression source changed after open.", EXIT_CONFLICT, {"sources": mismatches})
    return resolved
...
def brief(project, invocation_id: str) -> dict[str, Any]:
    ...
    if record["reaction"] != REACTION or record["protocol_version"] != protocol()["version"]:
        raise IsotopeError("invocation-mismatch", "The invocation does not bind this Expression protocol.", EXIT_CONFLICT)
    if record["status"] not in ("created", "running"):
        raise IsotopeError("invocation-not-briefable", "Only an open Expression invocation can pull a brief.", EXIT_CONFLICT, {"status": record["status"]})
    ...
    return {
        ...
        "values": {
            "specimen": value,
            "goal": value["goal"],
            "outcome": {key: value_ for key, value_ in resolved["outcome"].items() if key != "expression"},
            ...
            "answers": answers,
            "result_destination": "/outcome/expression",
        },
        "record_command": f"python .isotope/bin/isotope.py agent record expression --invocation {record['id']} --input <json-file>",
    }
...
def _validate_readout(value: Any) -> dict[str, Any]:
    ...
    status = value["status"]
    if status == "complete":
        if value["abstract"] is None or value["questions"] or value["condition"] is not None or value["next_action"] is not None:
            raise IsotopeError("result-malformed", "A complete Expression readout carries the abstract only.", EXIT_MALFORMED)
    elif status == "needs-user":
        if not value["questions"] or value["abstract"] is not None or value["next_action"] is None:
            raise IsotopeError("result-malformed", "needs-user requires questions and a next action.", EXIT_MALFORMED)
    elif status == "blocked":
        if value["condition"] is None or value["next_action"] is None or value["abstract"] is not None:
            raise IsotopeError("result-malformed", "blocked requires condition, facts, and next_action.", EXIT_MALFORMED)
    elif value["next_action"] is None or value["abstract"] is not None:
        raise IsotopeError("result-malformed", f"{status} requires a next action and no abstract.", EXIT_MALFORMED)
    return value


def _expression_entity(project, invocation: dict[str, Any], resolved: dict[str, Any], value: dict[str, Any]) -> dict[str, Any]:
    targets = []
    for target in resolved["outcome"]["doc_targets"]:
        _, source = isotope_docs.section(project, target["path"], target["section_id"])
        targets.append({"concept": target["concept"], "path": target["path"], "section_id": target["section_id"], "revision": source["revision"]})
    return {
...
def record(project, invocation_id: str, readout: Any) -> dict[str, Any]:
    ...
    if record_value["reaction"] != REACTION or record_value["protocol_version"] != protocol()["version"]:
        raise IsotopeError("invocation-mismatch", "The invocation does not bind Expression.", EXIT_CONFLICT)
    if os.environ.get("ISOTOPE_HOST") != record_value["host"]:
        raise IsotopeError("authority-unavailable", "The active native host does not match the Expression invocation.", EXIT_REFUSED, {"host": record_value["host"]})
    ...
    if record_value["status"] == "complete" and record_value["result"] is not None:
        ...
        entity = (specimen.get("outcome") or {}).get("expression")
        if entity is None or entity.get("abstract") != value["abstract"]:
            raise IsotopeError("expression-race", "The invocation already recorded a different Expression result.", EXIT_CONFLICT, {"invocation": invocation_id})
        compact = specimens.record_expression(project, invocation_id, entity=entity, source_guard=lambda: None)
    ...
    if status == "blocked":
        facts = {**value["facts"], "expression_state": revision(record_value["source_revisions"])}
    ...
    entity = _expression_entity(project, record_value, resolved, value)
    compact = specimens.record_expression(
        project,
        invocation_id,
        entity=entity,
