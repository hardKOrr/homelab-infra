"""Native Analyze protocol mapping, briefing, and durable structure-and-promote recording."""
...
def protocol() -> dict[str, Any]:
    ...
    if relative != "analyze/protocol.json":
        raise IsotopeError("protocol-invalid", "The Analyze protocol is absent from the reaction catalog.", EXIT_MALFORMED)
    ...
    if value.get("id") != REACTION or value.get("version") != "1" or value.get("transport") != "native-only":
        raise IsotopeError("protocol-invalid", "The Analyze protocol root is invalid.", EXIT_MALFORMED)
    return value
...
def _select_option(project, host: str, model: str | None) -> str:
    ...
    if len(choices) != 1:
        raise IsotopeError(
            ...
            "The requested host/model does not resolve to one enabled Analyze option.",
            EXIT_REFUSED,
...
def _coordinates(slug: str | None) -> dict[str, Any]:
    if slug is None:
        raise IsotopeError("usage", "Analyze requires the matter specimen slug.", EXIT_USAGE)
    return {"slug": slug}
...
def _source_revisions(project, specimen_revision: str, cultures: dict[str, list[str]], doc_revisions: dict[str, str]) -> dict[str, str]:
...
def _resolve(project, coordinates: dict[str, Any]) -> dict[str, Any]:
    ...
    if located.stage != "matter":
        raise IsotopeError("wrong-stage", "Analyze structures and promotes a matter specimen.", EXIT_REFUSED, {"slug": coordinates["slug"], "stage": located.stage})
    ...
    return {
        ...
        "cultures": cultures,
        "source_revisions": _source_revisions(project, specimen_revision, cultures, doc_revisions),
    }
    ...
    try:
        ...
        if after is not None:
            ...
            if predecessor["status"] == "blocked" and predecessor["blocking_condition"] is not None:
                ...
                if facts.get("analyze_state") == revision(resolved["source_revisions"]):
...
def open_invocation(project, slug: str | None, *, host: str, model: str | None, after: str | None) -> dict[str, Any]:
    ...
    if result["state"] != "ready":
        raise IsotopeError("agent-not-ready", "Analyze cannot open until inspection is ready.", EXIT_REFUSED, result)
...
def _assert_sources(project, invocation: dict[str, Any]) -> dict[str, Any]:
    ...
    if mismatches:
        raise IsotopeError("stale-analyze-source", "A frozen Analyze source changed after open.", EXIT_CONFLICT, {"sources": mismatches})
    return resolved
...
def brief(project, invocation_id: str) -> dict[str, Any]:
    ...
    if record["reaction"] != REACTION or record["protocol_version"] != protocol()["version"]:
        raise IsotopeError("invocation-mismatch", "The invocation does not bind this Analyze protocol.", EXIT_CONFLICT)
    if record["status"] not in ("created", "running"):
        raise IsotopeError("invocation-not-briefable", "Only an open Analyze invocation can pull a brief.", EXIT_CONFLICT, {"status": record["status"]})
    ...
    return {
        ...
        "values": {
            ...
            "answers": answers,
            "result_destination": specimens.culture_relative("flux", record["coordinates"]["slug"]),
        },
        "record_command": f"python .isotope/bin/isotope.py agent record analyze --invocation {record['id']} --input <json-file>",
    }
...
def _validate_readout(value: Any) -> dict[str, Any]:
    ...
    status = value["status"]
    if status == "complete":
        if value["analysis"] is None or value["questions"] or value["condition"] is not None or value["next_action"] is not None:
            raise IsotopeError("result-malformed", "A complete Analyze readout carries the analysis fields only.", EXIT_MALFORMED)
    elif status == "needs-user":
        if not value["questions"] or value["analysis"] is not None or value["next_action"] is None:
            raise IsotopeError("result-malformed", "needs-user requires questions and a next action.", EXIT_MALFORMED)
    elif status == "blocked":
        if value["condition"] is None or value["next_action"] is None or value["analysis"] is not None:
            raise IsotopeError("result-malformed", "blocked requires condition, facts, and next_action.", EXIT_MALFORMED)
    elif value["next_action"] is None or value["analysis"] is not None:
        raise IsotopeError("result-malformed", f"{status} requires a next action and no analysis payload.", EXIT_MALFORMED)
    return value
...
def record(project, invocation_id: str, readout: Any) -> dict[str, Any]:
    ...
    if record_value["reaction"] != REACTION or record_value["protocol_version"] != protocol()["version"]:
        raise IsotopeError("invocation-mismatch", "The invocation does not bind Analyze.", EXIT_CONFLICT)
    if os.environ.get("ISOTOPE_HOST") != record_value["host"]:
        raise IsotopeError("authority-unavailable", "The active native host does not match the Analyze invocation.", EXIT_REFUSED, {"host": record_value["host"]})
    ...
    if status == "blocked":
        facts = {**value["facts"], "analyze_state": revision(record_value["source_revisions"])}
    ...
    compact = specimens.record_analyze(
        project,
        invocation_id,
