...
net='"cidr":"10.20.0.0/20","gateway":"10.20.0.1"'

# Pool-less: the flat walk still works, so an existing config keeps its behaviour.
expect 10.20.0.10 "{$net,\"offset\":10,\"in_use\":[]}"
expect 10.20.0.12 "{$net,\"offset\":10,\"in_use\":[\"10.20.0.10\",\"10.20.0.11\"]}"
...
expect 10.20.0.2 "{$net,\"offset\":1,\"in_use\":[]}"

# Reserved spans hold whether or not Proxmox knows about the device.
expect 10.20.0.40 \
  "{$net,\"offset\":10,\"reserved\":[\"10.20.0.10-10.20.0.39\"],\"in_use\":[]}"
expect 10.20.0.16 "{$net,\"offset\":10,\"reserved\":[\"10.20.0.8/29\"],\"in_use\":[]}"

# A pool confines allocation to its own span, whatever the flat walk would have done.
pool='"pool":{"name":"media","range":"10.20.2.0-10.20.2.63"}'
expect 10.20.2.0 "{$net,$pool,\"offset\":10,\"in_use\":[\"10.20.0.10\"]}"
expect 10.20.2.2 "{$net,$pool,\"in_use\":[\"10.20.2.0\",\"10.20.2.1\"]}"
...
refuse "pool 'media'" \
  "{$net,\"pool\":{\"name\":\"media\",\"range\":\"10.20.2.0-10.20.2.1\"},\"in_use\":[\"10.20.2.0\",\"10.20.2.1\"]}"
...
expect 10.20.0.50 "{$net,\"pin\":\"10.20.0.50\",\"offset\":10,\"in_use\":[]}"
refuse "already held" "{$net,\"pin\":\"10.20.0.50\",\"in_use\":[\"10.20.0.50\"]}"
refuse "reserved" "{$net,\"pin\":\"10.20.0.1\"}"
refuse "outside" "{$net,\"pin\":\"10.20.9.9\",$pool}"
...
refuse "either held by a guest or reserved" \
  "{$net,\"offset\":10,\"max_hosts\":2,\"in_use\":[\"10.20.0.10\",\"10.20.0.11\"]}"

# Malformed input is refused, never guessed at.
refuse "not a subnet" '{"cidr":"10.20.0.0/33"}'
refuse "no cidr" '{}'
refuse "ends before it begins" \
  "{$net,\"pool\":{\"name\":\"backwards\",\"range\":\"10.20.0.60-10.20.0.40\"}}"
