...
| `proxmox.devices` | optional by default; required by an attachment request | map of `<physical-device-name>: {mode: shared\|dedicated, identifiers: [<exact host path, PCI address, or USB mapping>]}`. One entry names one physical device on `proxmox.node`; list every identifier that can refer to that unit (for example both an iGPU render node and its PCI address). Attachment seams reject a mode mismatch before inspecting guest state; the current mode gate covers PCI, shared-device, and the LXC USB resource-mapping attachment seams, so each rejects a device not declared with its requested mode. |
