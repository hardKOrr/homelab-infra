...
Durable engineering knowledge learned by operating this platform: failure shapes that
passed every gate, and structural facts that live nowhere else. Normative contracts live in
[`specs/`](specs/README.md) and [`../ansible/vars/CONTRACT.md`](../ansible/vars/CONTRACT.md);
current work lives in [GitHub Issues](https://github.com/hardKOrr/homelab-infra/issues).

Add to this file only when a future change would be wrong without the fact. Numbers such as
"slice 303" name retired work records, preserved in the
[`docs/meta/` archive at commit `056d836`](https://github.com/hardKOrr/homelab-infra/tree/056d836/docs/meta).
...
When work is called "built" and no execution is on record, the honest status is "unverified".
...
authoritative source for the `homelabinfra_infra` topology, and §6 for known conflicts.
What follows is the structural knowledge that lives nowhere else.
...
## Integrations with no live target

`routing.proxy: nginx` (Nginx Proxy Manager wiring), `dns: pihole` (Pi-hole v6+ wiring), and
the Semaphore `project.json` are code-complete and gate-green, but this lab runs Caddy,
...
## The `pve-cluster-1` lab was retired on 2026-09-23

Every platform-owned guest, template, and Proxmox object on `pve-cluster-1` was destroyed
on 2026-09-23. That includes the runner at `192.168.11.10`, Vaultwarden, PBS, and the k3s
cluster, and no data was kept. Live-lab work now targets a new cluster. Execution IDs,
addresses, and "observed on the lab" evidence recorded before that date describe a lab that
no longer exists. An open Observation issue whose evidence came from it has to be re-run on
the new cluster. Its old evidence does not carry over, and nothing at the old addresses will
answer.
