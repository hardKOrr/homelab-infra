...
**Blocks:** 012 (runner onboarding); any honest claim that this repo is shareable or reproducible
**Related:** 013 (Vaultwarden admin token self-capture), 014 (Vaultwarden as the generated-secret store)
...
- [ ] `bash rundeck/bootstrap-rundeck.sh` on a bare Proxmox node produces a runner whose
      `config/proxmox.yml` and `config/infrastructure.yml` exist and are complete, with no
      human editing either file
- [ ] That `config/proxmox.yml` contains **no secret**, and Lab Status runs green with
      `PROXMOX_API_TOKEN` supplied only from Key Storage
- [ ] The Proxmox credential used by the platform is `homelab-infra@pve`, not `root@pam`,
      and its secret exists in exactly one place
- [ ] The runner LXC carries the `homelab-infra` tag, appears in `Lab Status`, and its vmid
      is present in the PVE backup job created by `configure-pbs.yml`
- [ ] `config/apps/rundeck.yml` and the `runner` registry key describe the running host
- [x] `Configure App` writes `config/apps/<instance>.yml`, the job log shows a unified diff
      of the change, and the previous content is present under `config/apps/.backups/`
      — **verified on the workstation, not yet on the runner** (see Verified locally below)
- [x] `Get Config` returns the authored file with `admin_token` redacted; with
      `instance=radarr-4k` it returns one file — **verified on the workstation**
- [x] A round trip holds: `Configure App` writes an instance, `Get Config` returns it, and
      the returned YAML carries what the survey specified — **verified on the workstation**
- [x] `config-doctor` on an empty `config/` names every required key in one run, exits
      non-zero, and mutates nothing — **verified on the workstation**
- [ ] `config-doctor` on the live runner's populated `config/` exits zero
- [ ] `Bootstrap Platform` aborts at the doctor play — not mid-provision — when a required
      key is missing
- [ ] Restoring the runner LXC from PBS yields a working runner with its `config/` intact
