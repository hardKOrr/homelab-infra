...
Two secrets outside Vaultwarden — they are the credentials *for* the things that would otherwise hold them:
- `PROXMOX_API_TOKEN` — Proxmox connection. Operator-supplied; staged in Rundeck Key Storage / Semaphore env, reaching a run via `/etc/homelab-infra/secrets.env`.
- `VAULTWARDEN_ADMIN_TOKEN` — platform-generated on Vaultwarden's first deploy, written to the token sink (`/etc/homelab-infra/secrets.d/vaultwarden.env`, or `config/.generated/vaultwarden.env` off-runner). See CONTRACT.md §5.

Bootstrap chicken-and-egg, resolved: Vaultwarden deploys first with no prior secret. The deploy generates its admin token and writes it to the sink; the bootstrap gate reads it back and continues. **One pass, no paste, no re-run** (slice 013). The token is printed to the log only when the sink write fails.

Everything else is auto-generated by bootstrap. **Today it lands in `config/.generated/facts.yml` on the runner, not in Vaultwarden** — moving it into the vault is slice 014, still open. Do not describe the vault as the live store for generated secrets until that ships.
