...
#   wsl bash -lc 'bash .claude/gate/test.sh'          # narrow to the working tree's changes
#   wsl bash -lc 'bash .claude/gate/test.sh --all'    # full sweep
#
# A clean tree, an unreadable git, or a change to anything a playbook consumes all resolve
# to the full sweep — see .claude/gate/lib-scope.sh for why the narrowing only ever errs
